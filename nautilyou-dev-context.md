...
## Filtre enfant (contenu adulte) par listes publiques (2026-09-24)

Demande utilisateur : un interrupteur « filtre actif » dans les options WEB avec des cases à cocher pour choisir les listes, **UT1 cochée par défaut**.
- **Listes** (vérifiées : URL, format, taille, licence) : `ut1-adult` (Université Toulouse Capitole, archive tar.gz, **4,6 M domaines**, CC BY-SA 4.0), `blp-porn` (The Block List Project, 953 k, domaine public), `stevenblack-porn` (`alternates/porn-only/hosts`, 77 k, MIT), `hagezi-nsfw` (`wildcard/nsfw-onlydomains.txt`, 75 k, GPL-3.0), et `cloudflare-family` (DNS amont `1.1.1.3` au lieu de `1.1.1.1`, sans liste ; `pornhub.com` renvoie `0.0.0.0`). OISD écartée (licence peu claire). Rien n'est stocké dans le dépôt : **chaque PC télécharge les listes lui-même** depuis leur source.
- **Configuration** : `web.childFilter = { enabled, lists[] }` (défaut `{ false, ["ut1-adult"] }` dans `server/src/config.js` ; les anciennes configurations sans ce champ reçoivent la même valeur côté client et dashboard). Les identifiants sont dupliqués dans `dashboard-ui/src/blocklists.js` et `client/NautilyouService/BlocklistStore.cs` (`BlocklistCatalog`) : à garder alignés.
- **Client** (`BlocklistStore.cs`, nouveau) : télécharge, convertit (formats UT1 tar.gz, hosts, liste de domaines) et met en cache dans `%ProgramData%\Nautilyou\blocklists`, retéléchargement au plus **une fois par semaine**, copie locale conservée si le téléchargement échoue (fonctionne hors ligne). Les domaines sont stockés en **empreintes 64 bits triées** (FNV-1a) : 5,7 M domaines = **52 Mo** en mémoire (contre plus de 250 Mo en texte), rechargement depuis le cache en 26 ms, téléchargement + tri d'UT1 en 4,5 s. Collision négligeable (~1 sur 10^12 par requête). `DnsFilterServer` : `SetChildFilter`, recherche du domaine et de ses suffixes (`cdn.exemple.com` → `exemple.com`) par recherche binaire ; `Worker.ApplyChildFilterAsync` (au démarrage depuis le cache de config, à chaque config reçue, puis rafraîchi au moins toutes les 24 h ; une config plus récente l'emporte pendant un téléchargement).
- **Règles** : le filtre ne s'applique qu'en **mode liste noire** (en liste blanche, tout est déjà bloqué sauf les sites listés) ; la liste noire de l'utilisateur prime ; en mode liste noire la **liste blanche devient une liste d'exceptions** au filtre enfant (libellés adaptés dans le dashboard) ; une demande d'accès approuvée pour un site bloqué par une liste l'ajoute aux exceptions (`requests.js`). L'entrée renvoyée pour la notification/la demande est le suffixe trouvé dans la liste.
- **Dashboard** : section « Filtre enfant (contenu adulte) » dans la configuration WEB (interrupteur, cases à cocher avec description et licence, UT1 « recommandée »), ligne « Filtre enfant : activé (N listes) » sur la carte WEB. Testé dans le navigateur (activation, ajout d'une liste, enregistrement, affichage sur la carte).
- **Testé avec les vraies listes** (port de test) : filtre désactivé, tout passe ; activé, `pornhub.com`, `fr.pornhub.com`, `xhamster.com` bloqués (sinkhole), `wikipedia.org`, `lemonde.fr`, `example.com` autorisés ; exception « pornhub.com » en liste blanche l'autorise (et ses sous-domaines) ; mode liste blanche : les listes ne s'appliquent pas ; DNS famille seul : `0.0.0.0` reçu pour un site adulte.
- **Limites** : aucune liste ne bloque 100 % des sites ; sites adultes sur plateformes généralistes (Reddit, X…) non filtrés par domaine ; les sites bloqués par le DNS famille (`0.0.0.0`) n'affichent pas la notification « site bloqué » ; pas de recherche sécurisée (SafeSearch) forcée ; pas d'écran d'état des listes (dernière mise à jour, nombre de domaines) dans le dashboard. **Pas encore testé de bout en bout avec le vrai Service et le vrai serveur.**
